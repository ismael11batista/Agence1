...
function formatarEmail() {
    const texto = document.getElementById('inputText').value;
    // Utiliza a expressão regular para capturar o e-mail, independente de estar escrito como "E-mail" ou "Email"
    const emailRegex = /E-mail: (.+)|Email: (.+)/i;
    const emailMatch = texto.match(emailRegex);
    if (emailMatch) {
        // Captura o e-mail e o formata para letras minúsculas
        const email = (emailMatch[1] || emailMatch[2]).toLowerCase();
        // Atualiza a variável global com o e-mail formatado
        EmailFormatado = email;
        // (Opcional) Copia o e-mail formatado para a área de transferência
